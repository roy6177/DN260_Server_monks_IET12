...
class WCFM_Marketplace {
  ...
  function wcfmmp_order_total_commission( $order_id ) {
    ...
    $discount_total = $order_due[0]->discount_amount;
					...
					foreach( $tax_breakups as $tax_breakup_is => $tax_breakup ) {
						...
						<tr>
							...
							<td class="total" style="text-align:center; <?php echo $td_style; ?>">
								<div class="view">
									<?php 
										if( round( $tax_breakup['total'], 2 ) != round( $tax_breakup['subtotal'], 2 ) ) {
			...
			<?php if( apply_filters( 'wcfm_is_allow_vendor_order_details_tax_breakup_total', true ) ) { ?>
				...
				<tr>
					...
					<td class="total" style="text-align:center; <?php echo $td_style; ?>">
						<div class="view">
							<?php 
								if( round( $total_tax, 2 ) != round( $order_due[0]->tax, 2 ) ) {
		...
		<?php if ( $get_shipping && $order->get_formatted_shipping_address() ) { ?>
			...
			<tr>
				...
				<td class="total" style="text-align:center; <?php echo $td_style; ?>">
					<div class="view">
					  <?php 
						if( round( $total_shipping, 2 ) != round( $order_due[0]->shipping, 2 ) ) {
				...
				<?php if( apply_filters( 'wcfm_is_allow_vendor_order_details_tax_breakup_total', true ) ) { ?>
					...
					<tr>
						...
						<td class="total" style="text-align:center; <?php echo $td_style; ?>">
							<div class="view">
								<?php 
								if( round( $shipping_tax, 2 ) != round( $order_due[0]->shipping_tax_amount, 2 ) ) {
		...
		<?php
		if( wcfm_vendor_has_capability( $this->vendor_id, 'view_commission' ) && !in_array( $order_status, array( 'failed', 'cancelled', 'refunded', 'request', 'proposal', 'proposal-sent', 'proposal-expired', 'proposal-rejected', 'proposal-canceled', 'proposal-accepted' ) ) ) {
