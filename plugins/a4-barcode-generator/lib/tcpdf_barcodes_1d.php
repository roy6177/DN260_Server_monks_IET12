...
class TCPDFBarcode
{
    ...
    protected function encode_code39_ext($code)
    {
        ...
        $clen = strlen($code);
        for ($i = 0; $i < $clen; ++$i) {
            if (ord($code[$i]) > 127) { // $code{$i }
                return false;
            }
            $code_ext .= $encode[$code[$i]]; // $code{$i }
        }
        return $code_ext;
    }
    /**
    ...
    protected function checksum_code39($code)
    {
        ...
        $clen = strlen($code);
        for ($i = 0; $i < $clen; ++$i) {
            $k = array_keys($chars, $code[$i]); // $code{$i }
            $sum += $k[0];
        }
        $j = ($sum % 43);
        return $chars[$j];
    ...
    protected function binseq_to_array($seq, $bararray)
    {
        ...
        for ($i = 0; $i < $len; ++$i) {
            $w += 1; // $seq{$i}
            if (($i == ($len - 1)) or (($i < ($len - 1)) and ($seq[$i] != $seq[($i + 1)]))) { // $seq{($i+1)}
                if ($seq[$i] == '1') { // $seq{$i}
                    $t = true; // bar
                } else {
                    $t = false; // space
    ...
    protected function barcode_i25($code, $checksum = false)
    {
        ...
        for ($i = 0; $i < $clen; $i = ($i + 2)) {
            $char_bar = $code[$i]; // $code{$i }
            $char_space = $code[$i + 1]; // $code{$i+1}
            ...
            for ($s = 0; $s < $chrlen; $s++) {
                $seq .= $chr[$char_bar][$s] . $chr[$char_space][$s]; // $chr[$char_bar]{$s} , $chr[$char_space]{$s}
            }
            $seqlen = strlen($seq);
            for ($j = 0; $j < $seqlen; ++$j) {
                if (($j % 2) == 0) {
    ...
    protected function barcode_c128($code, $type = '')
    {
        ...
        switch (strtoupper($type)) {
            ...
            case 'C':{ // MODE C
                    ...
                    for ($i = 0; $i < $len; $i += 2) {
                        $chrnum = $code[$i] . $code[$i + 1]; // $code{$i } , $code{$i+1}
                    ...
                    foreach ($sequence as $key => $seq) {
                        switch ($seq[0]) {
                            ...
                            case 'C':{
                                    ...
                                    for ($i = 0; $i < $seq[2]; $i += 2) {
                                        $chrnum = $seq[1][$i] . $seq[1][$i + 1]; // $seq[1]{$i} , $seq[1]{$i+1}
                                        $code_data[] = intval($chrnum);
                                    }
                                    break;
                                }
    ...
    protected function barcode_eanupc($code, $len = 13)
    {
        ...
        if ($code_len == $data_len) {
            // add check digit
            $code .= $r;
        } elseif ($r !== intval($code[$data_len])) { // $code{$data_len}
            // wrong checkdigit
            return false;
        }
        if ($len == 12) {
        ...
        if ($upce) {
            ...
            for ($i = 0; $i < 6; ++$i) {
                $seq .= $codes[$p[$i]][$upce_code[$i]]; // $upce_code{$i}
            }
            ...
            if ($len == 8) {
                for ($i = 0; $i < $half_len; ++$i) {
                    $seq .= $codes['A'][$code[$i]]; // $code{$i }
                }
            } else {
                $p = $parities[$code[0]];
                for ($i = 1; $i < $half_len; ++$i) {
                    $seq .= $codes[$p[$i - 1]][$code[$i]]; // $code{$i }
                }
            }
            $seq .= '01010'; // center guard bar
            for ($i = $half_len; $i < $len; ++$i) {
                $seq .= $codes['C'][$code[$i]]; // $code{$i }
            }
    ...
    protected function barcode_rms4cc($code, $kix = false)
    {
        ...
        if ($notkix) {
            ...
            $col = 0;
            for ($i = 0; $i < $len; ++$i) {
                $row += $checktable[$code[$i]][0]; // $code{$i }
                $col += $checktable[$code[$i]][1]; // $code{$i }
            }
            $row %= 6;
            $col %= 6;
        ...
        for ($i = 0; $i < $len; ++$i) {
            for ($j = 0; $j < 4; ++$j) {
                switch ($barmode[$code[$i]][$j]) { // $code{$i }
                    case 1:{
                            $p = 0;
                            $h = 2;
                            break;
    ...
    protected function barcode_codabar($code)
    {
        ...
        $len = strlen($code);
        for ($i = 0; $i < $len; ++$i) {
            if (!isset($chr[$code[$i]])) { // $code{$i }
                return false;
            }
            $seq = $chr[$code[$i]]; // $code{$i }
            for ($j = 0; $j < 8; ++$j) {
                if (($j % 2) == 0) {
                    $t = true; // bar
                } else {
    ...
    public function hex_to_dec($hex)
    {
        ...
        for ($pos = ($len - 1); $pos >= 0; --$pos) {
            $dec = bcadd($dec, bcmul(hexdec($hex[$pos]), $bitval)); // $hex{$pos}
