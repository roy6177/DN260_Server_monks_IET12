...
foreach (
	...
	update_site_option('wpnotif_purchasecode', '8699958a-77f3-4db8-9422-126b0836e1c5');
    update_site_option('wpnotif_unr',time());
    update_site_option('wpnotif_license_type',1);
...
final class WPNotif
{
    ...
    {
    	update_option('wpnotif_purchasecode','B5E0B5F8DD8689E6ACA49DD6E6E1A930');
update_option('wpnotif_license_type', 1); delete_option('wpnotif_purchasefail');
        $this->init_hooks();
    ...
    public static function enqueue_scripts()
    {
        ...
        wp_enqueue_script('select2-full', plugins_url('/assets/js/select2.min.js', __FILE__), array('jquery'), null);
        ...
        <tr>
            ...
            <td class="wpnotif-gs-gatway-select-td">
                ...
                <select class="wpnotif_gateway" <?php echo $gatewayAttributes; ?> autocomplete="off">
                    ...
                    foreach ($gateway_groups as $group_name => $gateway_group) {
                        ...
                        foreach ($gateway_group as $name => $details) {
                            ...
                            echo '<option data-addon="' . $require_addon . '" data-value="' . $value . '" value="' . $value . '" ' . $sel . ' han="' . strtolower(str_replace(array(
                                    ".",
                                    " "
                ...
                if (!$additional_gateway_installed) { ?>
                    <p class="wpnotif_desc require_addon_text"><?php esc_html_e('Please install Additional Gateways addon to use this as SMS gateway'); ?>
...
function wpnotif_filter_update_checks($queryArgs)
{
    ...
    $queryArgs['license_key'] = '1234567890';


    $queryArgs['request_site'] = 'https://google.com';
