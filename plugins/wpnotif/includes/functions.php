...
final class WPNotif_Handler
{
    ...
    public function user_consent_ui($user_id, $show_heading)
    {
        ...
        if (is_user_logged_in()) {
            ...
            } else if ($default_checked && $whatsapp_notifications_consent != -1 && !is_admin() || ($default_checked && is_checkout())) {
                $whatsapp_notifications = 'checked';
            }

            if ($sms_notifications_consent == 1) {
                $sms_notifications = 'checked';
            } else if ($default_checked && $sms_notifications != -1 && !is_admin() || ($default_checked && is_checkout())) {
                $sms_notifications = 'checked';
            }


            if (!empty($sms_notifications) && !empty($whatsapp_notifications) || ($default_checked && is_checkout())) {
        ...
        if (is_array($placeholders)) {
            ...
            if (isset($placeholders[1])) {
                foreach ($placeholders[1] as $placeholder) {
                    ...
                    } else if ($order != null && (strpos($placeholder, 'order-') === 0 || strpos($placeholder, 'post-') === 0)) {
    ...
    public static function get_customer_mobile($user_id, $data, $is_admin = false)
    {
        ...
        if ($data['type'] != 'order' && $user_id != 0 || $is_admin) {
            ...
            if ($digits_mobile != null && !empty($dig_mobile)) {
