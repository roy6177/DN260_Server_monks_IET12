...
<message><sender>' . $sender . '</sender><text>' . $messagetemplate . '</text><abonent phone="' . $phone . '" /></message></request>';
            ...
            case 900:
                ...
                if (sizeof($attrs) == 1) {
                    $attrs = implode(",", $attrs);
                    $data = str_replace(array('{to}', '{sender_id}', '{message}'), array($to, $sender_id, $messagetemplate), $attrs);
                } else {
                    $data = array();
                    foreach ($attrs as $attr) {
                        ...
                        $attr_value = '';
                        if (isset($params[1]))
                            $attr_value = str_replace(array('{to}', '{sender_id}', '{message}'), array($to, $sender_id, $messagetemplate), $params[1]);
