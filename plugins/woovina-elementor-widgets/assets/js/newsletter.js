(function($) {
	var WidgetwewNewsletterHandler = function($scope, $) {

		var $form = $scope;

		function wewisValidEmailAddress(emailAddress) {
	    ...
	    $form.find('#mc-embedded-subscribe-form').on('submit', function(e) {
			...
			var $ajaxurl 	= $form.find('.wew-newsletter-form').data('ajaxurl'),
				element 	= $(this),
	        ...
	        } else if(! wewisValidEmailAddress(email)) {
	            valid = false;
	...
	$(window).on('elementor/frontend/init', function() {
		elementorFrontend.hooks.addAction('frontend/element_ready/wew-newsletter.default', WidgetwewNewsletterHandler);
	});
})(jQuery);