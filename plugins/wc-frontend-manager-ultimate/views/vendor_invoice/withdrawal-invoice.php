	  ...
	  if( !empty( $wi_commission_ids ) ) {
	  	foreach( $wi_commission_ids as $wi_commission_id ) {
				...
				if( !empty( $orders_details ) ) {
					...
					<tr>
						<td colspan="4">
							<h3><?php _e( 'Order #', 'wc-frontend-manager' ); echo esc_attr( $the_order->get_order_number() ); ?></h3>
							<?php
					...
					<tbody class="no-borders" style="float:right;">
					  <tr class="no-borders">
						...
						<tr class="no-borders">
							...
							<td style="text-align:right;float:right;padding-top:10px;"><?php echo wc_price( ( $total_commission_tax + $total_trans_charge + $total_aff_commission + $transaction->withdraw_amount ), array( 'currency' => $pay_currency ) ); ?></td>
						</tr>
						...
						<tr class="no-borders">
							...
							<td style="text-align:right;float:right;padding-top:10px;"><?php echo wc_price( $transaction->withdraw_amount, array( 'currency' => $pay_currency ) ); ?></td>
						</tr>
						<tr class="no-borders">
