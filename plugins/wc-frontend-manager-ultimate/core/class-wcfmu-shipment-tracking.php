      ...
      if( $tracking_code && $tracking_url ) {
				if( wcfm_is_vendor() ) {
					...
					if( apply_filters( 'wcfm_is_allow_itemwise_notification', true ) ) {
						...
						$wcfm_messages = apply_filters( 'wcfm_shipment_tracking_message', sprintf( __( 'Vendor <b>%s</b> has shipped <b>%s</b> to customer.<br/>Tracking Info : <a class="wcfm_dashboard_item_title" target="_blank" href="%s">%s</a>', 'wc-frontend-manager-ultimate' ), $shop_name, $order_id, $tracking_url, $tracking_code ), $tracking_code, $tracking_url, $order_id, '', $user_id );
						$WCFM->wcfm_notification->wcfm_send_direct_message( $user_id, 0, 0, 1, $wcfm_messages, 'shipment_tracking' );
