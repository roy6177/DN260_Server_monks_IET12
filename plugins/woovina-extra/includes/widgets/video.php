...
if(! class_exists('WooVina_Extra_Video_Widget')) {
	class WooVina_Extra_Video_Widget extends WP_Widget {
		...
		public function __construct() {
			parent::__construct(
				'woovina_video',
				esc_html__('&raquo; Video', 'woovina-extra'),
				array(
					'classname'   => 'widget-woovina-video video-widget',
					'description' => esc_html__('Easily to display any type of video.', 'woovina-extra'),
		...
		public function widget($args, $instance) {
			...
			echo $args['before_widget'];
				...
				if($video_url)  {
					...
					esc_html_e('You forgot to enter a video URL.', 'woovina-extra');
				}
				
				// Show video description if field isn't empty
				if($description) {
					echo '<div class="woovina-video-widget-description">'. do_shortcode($description) .'</div>';
				}
		...
		public function form($instance) {
			...
			$instance = wp_parse_args((array) $instance, array(
				'title'             => esc_attr__('Video', 'woovina-extra'),
				'video_url'         => '',
				'video_description' => '',
			)); ?>
			...
			<p>
				...
				<?php esc_html_e('Video URL ', 'woovina-extra'); ?></label>
				...
				<span style="display:block;padding:5px 0" class="description"><?php esc_html_e('Enter in a video URL that is compatible with WordPress\'s built-in oEmbed feature.', 'woovina-extra'); ?> <a href="http://codex.wordpress.org/Embeds" target="_blank"><?php esc_html_e('Learn More', 'woovina-extra'); ?></a></span>
			</p>

			<p>
				<label for="<?php echo esc_attr($this->get_field_id('video_description')); ?>"><?php esc_html_e('Description', 'woovina-extra'); ?></label>
...
register_widget('WooVina_Extra_Video_Widget');