 ...
 * Nav menu admin
 */
final class WE_Menu_Icons_Picker {
	...
	public static function init() {
		...
		add_filter('we_icon_picker_type_props', array(__CLASS__, '_add_extra_type_props_data'), 10, 3);
	}
	...
	public static function _load_nav_menus() {
		WE_Icon_Selector::instance()->load();
	...
	protected static function _get_menu_item_fields($meta) {
		$fields = array_merge(
			...
			WE_Menu_Icons_Settings::get_settings_fields($meta)
		);
	...
	public static function _fields($id, $item, $depth, $args) {
		$input_id      = sprintf('we-icons-%d', $item->ID);
		$input_name    = sprintf('we-icons[%d]', $item->ID);
		$menu_settings = WE_Menu_Icons_Settings::get_menu_settings(WE_Menu_Icons_Settings::get_current_menu_id());
		$meta          = WE_Menu_Icons_Meta::get($item->ID, $menu_settings);
		$fields        = self::_get_menu_item_fields($meta);
		?>
			<div class="field-icon description-wide we-icons-wrap" data-id="<?php echo json_encode($item->ID); ?>">
				<?php
					/**
					 * Add filter to allow to inject HTML before menu icons fields
					 */
					do_action('we_menu_icons_before_fields', $item, $depth, $args, $id);
				?>
				<p class="description submitbox">
					<label><?php esc_html_e('Icon:', 'woovina-extra') ?></label>
					<?php printf('<a class="_select">%s</a>', esc_html__('Select', 'woovina-extra')); ?>
					<?php printf('<a class="_remove submitdelete hidden">%s</a>', esc_html__('Remove', 'woovina-extra')); ?>
				</p>
				<div class="_settings hidden">
					<?php
					foreach ($fields as $field) {
						printf(
							'<label>%1$s: <input type="text" name="%2$s" class="_we-%3$s" value="%4$s" /></label><br />',
					...
					printf('<input type="hidden" class="_we-url" value="%s" />', esc_attr($meta['url']));
					?>
				</div>
				<?php
					/**
					 ...
					 */
					do_action('we_menu_icons_after_fields', $item, $depth, $args, $id);
				?>
			</div>
		<?php
	}
	...
	public static function _columns($columns) {
		$columns['icon'] = __('Icon', 'woovina-extra');
	...
	public static function _save($menu_id, $menu_item_db_id, $menu_item_args) {
		...
		// Sanitize
		if(! empty($_POST['we-icons'][ $menu_item_db_id ])) {
			$value = array_map(
				'sanitize_text_field',
				wp_unslash((array) $_POST['we-icons'][ $menu_item_db_id ])
			);
		} else {
			$value = array();
		}

		WE_Menu_Icons_Meta::update($menu_item_db_id, $value);
	}
