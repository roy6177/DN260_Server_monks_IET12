 ...
 *
 */
function we_icon_picker_get_icon_url($type, $id, $size = 'thumbnail') {
	$url = '';
 ...
 *
 */
function we_icon_picker_field($args, $echo = true) {
	$defaults = array(
		'id'    => '',
		'name'  => '',
		'value' => array(
		...
		'select' => sprintf('<a class="ipf-select">%s</a>', esc_html__('Select Icon', 'woovina-extra')),
		'remove' => sprintf('<a class="ipf-remove button hidden">%s</a>', esc_html__('Remove', 'woovina-extra')),
	);
	...
	$field .= sprintf(
		...
		esc_attr(we_icon_picker_get_icon_url($args['value']['type'], $args['value']['icon']))
	);
	$field .= '</div>';
