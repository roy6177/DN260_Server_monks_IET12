   ...
   */
		setIcon: function setIcon($el) {
			...
			$el.find('div._settings input').each(function () {
				var $input = $(this),
				    key = $input.attr('class').replace('_we-', ''),
				    value = $input.val();
   ...
   */
		updateField: function updateField(e) {
			var $el = $(e.currentTarget),
			    ...
			    type = $el.find('input._we-type').val(),
			    icon = $el.find('input._we-icon').val(),
			    url = $el.find('input._we-url').val(),
			    template;
			...
			if(miPicker.templates[type]) {
				...
				template = miPicker.templates[type] = wp.template('we-icons-item-field-preview-' + iconPicker.types[type].templateId);
			}
...
(function ($) {
  ...
  */
	$('#we-icons-settings-tabs').on('click', 'a.we-settings-nav-tab', function (e) {
		var $el = $(this).blur(),
		    $target = $('#' + $el.data('type'));
	...
	}).find('a.we-settings-nav-tab').first().click();

	// Settings meta box
	$('#we-icons-settings-save').on('click', function (e) {
		...
		$.ajax({
			...
			data: $('#we-icons-settings :input').serialize(),
...
var MenuIconsItemPreview = wp.media.View.extend({
	tagName: 'p',
	className: 'we-preview menu-item attachment-info',
	events: {
	...
	render: function render() {
		var frame = this.controller,
		    ...
		    template = 'we-icons-item-sidebar-preview-' + iconPicker.types[state.id].templateId + '-';
...
MenuIconsItemSettingField = wp.media.View.extend({
	...
	initialize: function initialize() {
		...
		this.template = wp.media.template('we-icons-settings-field-' + this.model.get('type'));
		this.model.on('change', this.render, this);
	},
...
var MenuIconsItemSettings = wp.media.view.PriorityList.extend({
	className: 'we-settings attachment-info',
