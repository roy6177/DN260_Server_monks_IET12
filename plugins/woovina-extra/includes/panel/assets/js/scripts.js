...
$j(document).ready(function(e) {
	...
	(function () {
		...
		var $items 				= $j('.woovina-modules .modules-inner').find('.column-wrap'),
		...
		$j('.woovina-modules .btn-switcher li a').on('click', function(event) {
		...
		$j(document).on('click', '#wvn-switch-all', function() {
			if($j(this).is(':checked')) {
			    $j(this).closest('.woovina-modules').find('input.wvn-checkbox').prop('checked', true);
			} else {
			    $j(this).closest('.woovina-modules').find('input.wvn-checkbox').prop('checked', false);
			}
		});
	}());
