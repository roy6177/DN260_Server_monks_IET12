(function($) {
	...
	var owpDemoImport = {
		...
		init: function() {
			...
			$('.wvn-search-input').on('keyup', function() {
				if(0 < $(this).val().length) {
					// Hide all items.
					$('.wvn-demo-wrap .themes').find('.theme-wrap').hide();

					// Show just the ones that have a match on the import name.
					$('.wvn-demo-wrap .themes').find('.theme-wrap[data-name*="' + $(this).val().toLowerCase() + '"]').show();
				} else {
					$('.wvn-demo-wrap .themes').find('.theme-wrap').show();
				}
			});

			// Prevent the popup from showing when the live preview button
			$('.wvn-demo-wrap .theme-actions a.button').on('click', function(e) {
			   e.stopPropagation();
			});

			// Get demo data
			$('.wvn-open-popup').click(function(e) {
				e.preventDefault();

				// Vars
				var $selected_demo 		= $(this).data('demo-id'),
					$loading_icon 		= $(this).find('div.preview-icon'),
					$disable_preview 	= $(this).find('div.preview-all');
		...
		categoriesFilter: function() {
			...
			var $items 				= $('.wvn-demo-wrap .themes').find('.theme-wrap'),
				fadeoutClass 		= 'wvn-is-fadeout',
				fadeinClass 		= 'wvn-is-fadein',
			...
			var fadeIn = function (category, dfd) {
				...
				$items.filter(filter).show().addClass('wvn-is-fadein');
			...
			$('.wvn-navigation-link').on('click', function(event) {
				...
				var $container = $('.wvn-demo-wrap .themes');
		...
		getDemoData: function(demo_name) {
			...
			// Get import data
			$.ajax({
				url: wvnDemos.ajaxurl,
				type: 'get',

				data: {
					action: 'wvn_ajax_get_import_data',
					demo_name: demo_name,
					security: wvnDemos.wvn_import_data_nonce
				},
			...
			$.ajax({
				...
				data: {
					action : 'wvn_ajax_get_demo_data',
					demo_name: demo_name,
					demo_data_nonce: wvnDemos.demo_data_nonce
				},
				...
				complete: function(data) {
					...
					// Vars
					var $loading_icon 		= $('div.preview-icon'),
						$disable_preview 	= $('div.preview-all');
		...
		runPopup: function(data) {
			...
			// Show popup
			$('#wvn-demo-popup-wrap').fadeIn();
			$(data.responseText).appendTo($('#wvn-demo-popup-content'));

			// Close popup
			$('.wvn-demo-popup-close, .wvn-demo-popup-overlay').on('click', function(e) {
				e.preventDefault();
			...
			$('.wvn-plugins-next').on('click', function(e) {
				e.preventDefault();
				
				// Hide step one
				$('#wvn-demo-plugins').hide();

				// Display step two
				$('#wvn-demo-import-form').show();

			});

			// if clicked on import data button
			$('#wvn-demo-import-form').submit(function(e) {
				e.preventDefault();

				// Vars
				var demo 	= $(this).find('[name="wvn_import_demo"]').val(),
					nonce 	= $(this).find('[name="wvn_import_demo_data_nonce"]').val(),
					contentToImport = [];
				...
				that.importContent({
					...
					isXML: $('#wvn_import_xml').is(':checked')
				});
			});
		...
		importContent: function(importData) {
			var that = this,
				...
				timerStart = Date.now(),
				ajaxData = {
					wvn_import_demo: importData.demo,
					wvn_import_demo_data_nonce: importData.nonce
				};

			this.allowPopupClosing = false;
			$('.wvn-demo-popup-close').fadeOut();
			...
			if(importData.contentToImport.length === 0) {				
				...
				setTimeout(function() {
					var error = $('.wvn-loader').find('.wvn-importing-failed');					
					if(error.length == 0) {
						$('.wvn-loader').hide();
						$('.wvn-last.wvn-success').show();
					}
					else {
						$('.wvn-last.wvn-error').show();
					}
				}, 1000);				
				...
				$.ajax({
					...
					type: 'post',
					data: {
						action: 'wvn_after_import',
						wvn_import_demo: importData.demo,
						wvn_import_demo_data_nonce: importData.nonce,
						wvn_import_is_xml: importData.isXML
					},
					complete: function(data) {}
				});

				this.allowPopupClosing = true;
				$('.wvn-demo-popup-close').fadeIn();
			...
			$('.wvn-import-status').append('<p class="wvn-importing">' + this.importData[ currentContent ]['loader'] + '</p>');
			...
			var ajaxRequest = $.ajax({
				...
				complete: function(data) {
					...
					if(data.status === 500 || data.status === 502 || data.status === 503) {
						$('.wvn-importing')
							.addClass('wvn-importing-failed')
							.removeClass('wvn-importing')
							.text(wvnDemos.content_importing_error + ' '+ data.status);
					} else if(data.responseText.indexOf('successful import') !== -1) {
						$('.wvn-importing').addClass('wvn-imported').removeClass('wvn-importing');
					} else {
			...
			importingLimit = setTimeout(function() {
				...
				$('.wvn-importing')
					.addClass('wvn-importing-failed')
					.removeClass('wvn-importing')
					.text(wvnDemos.content_importing_error);
			}, 15 * 60 * 1000);
		...
		closePopup: function() {
			...
			// Hide demo popup
			$('#wvn-demo-popup-wrap').fadeOut();

			// Remove content in the popup
			setTimeout(function() {
				$('#wvn-demo-popup-content').html('');
			}, 600);
		},
		...
		installNow: function(e) {
			...
			if('premium' == $type) {
				...
				type: 'POST',
				data: {
						action : 'wvn_ajax_install_plugin',
						slug: $slug,
						init: $init,
					},
				}).done(function(result) {
					$button.addClass('activate-now updating-message button-primary').removeClass('install-now').html(wvnDemos.button_activating);
					...
					setTimeout(function() {
						$.ajax({
							...
							type: 'POST',
							data: {
								action : 'wvn_ajax_required_plugins_activate',
								init   : $init,
							},
						}).done(function(result) {
							if(result.success) {
		...
		activatePlugins: function(e) {
			...
			$button.addClass('updating-message button-primary').html(wvnDemos.button_activating);
		...
		installSuccess: function(e, response) {
			...
			var $message = $('.wvn-plugin-' + response.slug).find('.button');
			...
			$message.removeClass('install-now installed button-disabled updated-message')
				...
				.html(wvnDemos.button_activating);
		...
		installError: function(e, response) {
			...
			var $card = $('.wvn-plugin-' + response.slug);
