...
if ( !isset( $post->ID ) && ! isset( $_GET['product_id'] ) ) {
    wp_die( esc_html__( 'Access Denied, No product found', 'woovina' ) );
}
...
if ( ! dokan_is_product_author( $post_id ) ) {
    wp_die( esc_html__( 'Access Denied', 'woovina' ) );
    exit();
}
							...
							<div class="dokan-dashboard-content dokan-product-edit">
								...
								<header class="dokan-dashboard-header dokan-clearfix">
									<h1 class="entry-title">
										<?php esc_html_e( 'Edit Product', 'woovina' ); ?>
										...
										<?php if ( $post->post_status == 'publish' ) { ?>
											<span class="dokan-right">
												<a class="dokan-btn dokan-btn-theme dokan-btn-sm" href="<?php echo esc_url( get_permalink( $post->ID ) ); ?>" target="_blank"><?php esc_html_e( 'View Product', 'woovina' ); ?></a>
											</span>
										<?php } ?>

										<?php if ( $_visibility == 'hidden' ) { ?>
											<span class="dokan-right dokan-label dokan-label-default dokan-product-hidden-label"><i class="fa fa-eye-slash"></i> <?php esc_html_e( 'Hidden', 'woovina' ); ?></span>
										<?php } ?>
									</h1>
								</header><!-- .entry-header -->
								...
								<div class="product-edit-new-container product-edit-container">
									<?php if ( Dokan_Template_Products::$errors ) { ?>
										<div class="dokan-alert dokan-alert-danger">
											...
											<?php foreach ( Dokan_Template_Products::$errors as $error) { ?>
												<strong><?php esc_html_e( 'Error!', 'woovina' ); ?></strong> <?php echo esc_html( $error ) ?>.<br>
											<?php } ?>
										</div>
									<?php } ?>
									...
									<?php if ( isset( $_GET['message'] ) && $_GET['message'] == 'success') { ?>
										<div class="dokan-message">
											...
											<strong><?php esc_html_e( 'Success!', 'woovina' ); ?></strong> <?php esc_html_e( 'The product has been saved successfully.', 'woovina' ); ?>

											<?php if ( $post->post_status == 'publish' ) { ?>
												<a href="<?php echo esc_url( get_permalink( $post_id ) ); ?>" target="_blank"><?php esc_html_e( 'View Product &rarr;', 'woovina' ); ?></a>
											<?php } ?>
										</div>
									<?php } ?>
										...
										if ( dokan_is_seller_enabled( get_current_user_id() ) ) { ?>
											<form class="dokan-product-edit-form" role="form" method="post">
												...
												<div class="dokan-form-top-area">
													...
													<div class="content-half-part dokan-product-meta">
														...
														<div class="dokan-form-group">
															...
															<label for="post_title" class="form-label"><?php esc_html_e( 'Title', 'woovina' ); ?></label>
															<?php dokan_post_input_box( $post_id, 'post_title', array( 'placeholder' => __( 'Product name..', 'woovina' ), 'value' => $post_title ) ); ?>
															<div class="dokan-product-title-alert dokan-hide">
																<?php esc_html_e( 'Please enter product title!', 'woovina' ); ?>
															</div>
														</div>
														...
														<?php if ( is_array( $product_types ) ): ?>
															<div class="dokan-form-group">
																<label for="product_type" class="form-label"><?php esc_html_e( 'Product Type', 'woovina' ); ?> <i class="fa fa-question-circle tips" aria-hidden="true" data-title="<?php esc_html_e( 'Choose Variable if your product has multiple attributes - like sizes, colors, quality etc', 'woovina' ); ?>"></i></label>
														...
														<div class="show_if_simple dokan-clearfix">
															...
															<div class="dokan-form-group dokan-clearfix dokan-price-container">
																...
																<div class="content-half-part regular-price">
																	<label for="_regular_price" class="form-label"><?php esc_html_e( 'Price', 'woovina' ); ?>
																		<span class="vendor-earning" data-commission="<?php echo esc_attr( dokan_get_seller_percentage( dokan_get_current_user_id(), $post_id ) ); ?>" data-commission_type="<?php echo esc_attr( dokan_get_commission_type( dokan_get_current_user_id(), $post_id ) ); ?>">( <?php esc_html_e( ' You Earn : ', 'woovina' ) ?><?php echo esc_html( get_woocommerce_currency_symbol() ); ?><span class="vendor-price">0.00</span> )</span>
																	</label>
																	<div class="dokan-input-group">
																		<span class="dokan-input-group-addon"><?php echo esc_html( get_woocommerce_currency_symbol() ); ?></span>
																		<?php dokan_post_input_box( $post_id, '_regular_price', array( 'class' => 'dokan-product-regular-price', 'placeholder' => __( '0.00', 'woovina' ) ), 'number' ); ?>
																	</div>
																</div>

																<div class="content-half-part sale-price">
																	<label for="_sale_price" class="form-label">
																		<?php esc_html_e( 'Discounted Price', 'woovina' ); ?>
																		<a href="#" class="sale_schedule <?php echo (($show_schedule) ? 'dokan-hide' : ''); ?>"><?php esc_html_e( 'Schedule', 'woovina' ); ?></a>
																		<a href="#" class="cancel_sale_schedule <?php echo ( ! $show_schedule ) ? 'dokan-hide' : ''; ?>"><?php esc_html_e( 'Cancel', 'woovina' ); ?></a>
																	</label>

																	<div class="dokan-input-group">
																		<span class="dokan-input-group-addon"><?php echo esc_html( get_woocommerce_currency_symbol() ); ?></span>
																		<?php dokan_post_input_box( $post_id, '_sale_price', array( 'class' => 'dokan-product-sales-price','placeholder' => __( '0.00', 'woovina' ) ), 'number' ); ?>
																	</div>
																</div>
															</div>
															...
															<div class="dokan-form-group dokan-clearfix dokan-price-container">
																<div class="dokan-product-less-price-alert dokan-hide">
																	<?php esc_html_e('Product price can\'t be less than the vendor fee!', 'woovina' ); ?>
																</div>
															</div>
															...
															<div class="sale_price_dates_fields dokan-clearfix dokan-form-group <?php echo ( ! $show_schedule ) ? 'dokan-hide' : ''; ?>">
																<div class="content-half-part from">
																	<div class="dokan-input-group">
																		<span class="dokan-input-group-addon"><?php esc_html_e( 'From', 'woovina' ); ?></span>
																		<input type="text" name="_sale_price_dates_from" class="dokan-form-control datepicker" value="<?php echo esc_attr( $_sale_price_dates_from ); ?>" maxlength="10" pattern="[0-9]{4}-(0[1-9]|1[012])-(0[1-9]|1[0-9]|2[0-9]|3[01])" placeholder="<?php esc_html_e( 'YYYY-MM-DD', 'woovina' ); ?>">
																	</div>
																</div>

																<div class="content-half-part to">
																	<div class="dokan-input-group">
																		<span class="dokan-input-group-addon"><?php esc_html_e( 'To', 'woovina' ); ?></span>
																		<input type="text" name="_sale_price_dates_to" class="dokan-form-control datepicker" value="<?php echo esc_attr( $_sale_price_dates_to ); ?>" maxlength="10" pattern="[0-9]{4}-(0[1-9]|1[012])-(0[1-9]|1[0-9]|2[0-9]|3[01])" placeholder="<?php esc_html_e( 'YYYY-MM-DD', 'woovina' ); ?>">
																	</div>
																</div>
														...
														<?php if ( dokan_get_option( 'product_category_style', 'dokan_selling', 'single' ) == 'single' ): ?>
															<div class="dokan-form-group">
																<label for="product_cat" class="form-label"><?php esc_html_e( 'Category', 'woovina' ); ?></label>
																<?php
																$product_cat = -1;
																$term = array();
																...
																$category_args =  array(
																	'show_option_none' => __( '- Select a category -', 'woovina' ),
																	'hierarchical'     => 1,
																	'hide_empty'       => 0,
																	'name'             => 'product_cat',
																	'id'               => 'product_cat',
															...
															?>
																<div class="dokan-product-cat-alert dokan-hide">
																	<?php esc_html_e('Please choose a category!', 'woovina' ); ?>
																</div>
															</div>
														<?php elseif ( dokan_get_option( 'product_category_style', 'dokan_selling', 'single' ) == 'multiple' ): ?>
															<div class="dokan-form-group">
																...
																echo str_replace( '<select', '<select data-placeholder="' . esc_html__( 'Select product category', 'woovina' ) . '" multiple="multiple" ', $drop_down_category ); // phpcs:ignore WordPress.XSS.EscapeOutput.OutputNotEscaped
																?>
															</div>
														<?php endif; ?>

														<div class="dokan-form-group">
															<label for="product_tag" class="form-label"><?php esc_html_e( 'Tags', 'woovina' ); ?></label>
															<?php
															...
															echo str_replace( '<select', '<select data-placeholder="' . esc_html__( 'Select product tags', 'woovina' ) . '" multiple="multiple" ', $drop_down_tags ); // phpcs:ignore WordPress.XSS.EscapeOutput.OutputNotEscaped
													...
													<div class="content-half-part featured-image">
														...
														<div class="dokan-feat-image-upload dokan-new-product-featured-img">
															...
															<div class="instruction-inside<?php echo esc_attr( $instruction_class ); ?>">
																...
																<a href="#" class="dokan-feat-image-btn btn btn-sm"><?php esc_html_e( 'Upload a product cover image', 'woovina' ); ?></a>
															</div>
															...
															<div class="image-wrap<?php echo esc_attr( $wrap_class ); ?>">
																...
																<?php } else { ?>
																	<img height="" width="" src="" alt="Empty ALT">
																<?php } ?>
															</div>
																			...
																			if ( $gallery ) {
																				foreach ($gallery as $image_id) {
																					...
																					<li class="image" data-attachment_id="<?php echo esc_attr( $image_id ); ?>">
																						<img src="<?php echo esc_url( $attachment_image[0] ); ?>" alt="Empty ALT">
																						<a href="#" class="action-delete" title="<?php esc_attr_e( 'Delete image', 'woovina' ); ?>">&times;</a>
																					</li>
																					<?php
																				}
																			}
																			?>
																			<li class="add-image add-product-images tips" data-title="<?php esc_html_e( 'Add gallery image', 'woovina' ); ?>">
												...
												<div class="dokan-product-short-description">
													<label for="post_excerpt" class="form-label"><?php esc_html_e( 'Short Description', 'woovina' ); ?></label>
												...
												<div class="dokan-product-description">
													<label for="post_content" class="form-label"><?php esc_html_e( 'Description', 'woovina' ); ?></label>
												...
												<input type="hidden" name="dokan_update_product" value="<?php esc_attr_e( 'Save Product', 'woovina' ); ?>"/>
												<input type="submit" name="dokan_update_product" class="dokan-btn dokan-btn-theme dokan-btn-lg dokan-right" value="<?php esc_attr_e( 'Save Product', 'woovina' ); ?>"/>
