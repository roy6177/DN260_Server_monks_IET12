...
// Text
$text = esc_html__('You Might Also Like', 'woovina');

// Apply filters for child theming
$text = apply_filters('woovina_related_posts_title', $text);

// Number of columns for entries
$woovina_columns = apply_filters('woovina_related_blog_posts_columns', absint(get_theme_mod('woovina_blog_related_columns', '3')));

// Term
$term_tax = get_theme_mod('woovina_blog_related_taxonomy', 'category');
...
// Query args
$args = array(
	'posts_per_page' => apply_filters('woovina_related_blog_posts_count', absint(get_theme_mod('woovina_blog_related_count', '3'))),
	'orderby'        => 'rand',
...
// Args
$args = apply_filters('woovina_blog_post_related_query_args', $args);

// Related query arguments
$woovina_related_query = new WP_Query($args);

// If the custom query returns post display related posts section
if($woovina_related_query->have_posts()) :

	// Wrapper classes
	$classes = 'clr';
	if('full-screen' == woovina_post_layout()) {
		$classes .= ' container';
	} ?>

	<?php do_action('woovina_before_single_post_related_posts'); ?>
	...
	<section id="related-posts" class="<?php echo esc_attr($classes); ?>">
		...
		<div class="woovina-row clr">
			...
			<?php foreach($woovina_related_query->posts as $post) : setup_postdata($post); ?>

				<?php $woovina_count++;

				// Disable embeds
				$show_embeds = apply_filters('woovina_related_blog_posts_embeds', false);
				...
				$classes[]	= woovina_grid_class($woovina_columns);
				...
				<article <?php post_class($classes); ?>>
					...
					<?php
					// Display post video
					if($show_embeds && 'video' == $format && $video = woovina_get_post_video_html()) : ?>
					...
					<?php
					// Display post audio
					elseif($show_embeds && 'audio' == $format && $audio = woovina_get_post_audio_html()) : ?>
					...
					elseif(has_post_thumbnail()) : ?>
						...
						<figure class="related-post-media clr">
							...
							<a href="<?php the_permalink(); ?>" class="related-thumb">
								...
								<?php
								// Image width
								$img_width  = apply_filters('woovina_related_blog_posts_img_width', absint(get_theme_mod('woovina_blog_related_img_width')));
								$img_height = apply_filters('woovina_related_blog_posts_img_height', absint(get_theme_mod('woovina_blog_related_img_height')));
								...
								<?php
									...
									// Images size
									if('full-width' == woovina_post_layout()
										|| 'full-screen' == woovina_post_layout()) {
										$size = 'medium_large';
									} else {
										$size = 'medium';
									}
				...
				<?php if($woovina_columns == $woovina_count) $woovina_count=0; ?>
	...
	<?php do_action('woovina_after_single_post_related_posts'); ?>
