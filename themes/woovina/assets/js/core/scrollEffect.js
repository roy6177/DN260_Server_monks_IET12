	...
	if(! $j('body').hasClass('single-product')
	    ...
	    $j('a.local[href*="#"]:not([href="#"]), .local a[href*="#"]:not([href="#"]), a.menu-link[href*="#"]:not([href="#"]), a.sidr-class-menu-link[href*="#"]:not([href="#"])').on('click', function() {
	    	...
	    	if(! $j(this).hasClass('omw-open-modal')
	        	...
	        	&& ! $j(this).parent().hasClass('wpl-link')) {

	        	var $href     				= $j(this).attr('href'),
				    $hrefHash 				= $href.substr($href.indexOf('#')).slice(1),
				    $target   				= $j('#' + $hrefHash),
					$adminbarHeight        	= woovinaGetAdminbarHeight(),
					$topbarHeight        	= woovinaGetTopbarHeight(),
					$stickyHeaderHeight    	= woovinaGetStickyHeaderHeight(),
...
// Admin bar height
function woovinaGetAdminbarHeight() {
	"use strict"
...
// Top bar height
function woovinaGetTopbarHeight() {
	"use strict"
...
// Header height
function woovinaGetStickyHeaderHeight() {
	"use strict"
