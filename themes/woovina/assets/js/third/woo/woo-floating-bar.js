(function($) {
    $(document).scroll(function() {
        ...
        $('.wvn-floating-bar button.button.top').on('click', function(e) {
        	e.preventDefault();
	...
	var owpFBAddToCartHandler = function() {
		$(document.body)
			.on('click', '.wvn-floating-bar .floating_add_to_cart_button', this.onAddToCart)
	...
	owpFBAddToCartHandler.prototype.onAddToCart = function(e) {
		...
		$.ajax ({
			...
			type:'POST',
			data:'action=woovina_add_cart_floating_bar&product_id=' + product_id + '&quantity=' + quantity,
