...
if(! class_exists('WooVina_Custom_Nav_Walker')) {
	class WooVina_Custom_Nav_Walker extends Walker_Nav_Menu {
		...
		public function start_lvl(&$output, $depth = 0, $args = array()) {
	        ...
	        if($depth === 0 && $this->megamenu != '' && 'full_screen' != woovina_header_style() && 'vertical' != woovina_header_style()) {
		...
		public function start_el(&$output, $item, $depth = 0, $args = array(), $id = 0) {
		    ...
		    if($depth === 0 && $this->has_children && 'vertical' != woovina_header_style()) {
			...
			if($item->category_post != '' && $item->object == 'category' && 'vertical' != woovina_header_style()) {
			...
			if(($item->template || $item->mega_template) && $this->megamenu != '' && 'vertical' != woovina_header_style()) {
				ob_start();
					include(WOOVINA_INC_DIR . 'walker/template.php');
					$template_content = ob_get_contents();
				ob_end_clean();
				$item_output .= $template_content;
			}

			if($item->megamenu_widgetarea && $this->megamenu != '' && 'vertical' != woovina_header_style()) {
				ob_start();
				...
				$output .= "\n<ul class=\"megamenu col-4 sub-menu\">\n";
					...
					if($item->category_post != '' && $item->object == 'category') {
						...
						foreach($sub_categories as $category) {
							...
							// Title
							$output .= '<h3 class="mega-cat-title">'. esc_html__('Latest in', 'woovina') .' '. get_cat_name($cat_id) .'</h3>';
		...
		public function display_element($element, &$children_elements, $max_depth, $depth=0, $args, &$output) {
			...
			if(! empty($children_elements[$element->$id_field]) && ($depth == 0)
				...
				if(true == get_theme_mod('woovina_menu_arrow_down', true)) {
