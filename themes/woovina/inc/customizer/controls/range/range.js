wp.customize.controlConstructor['woovina-range'] = wp.customize.Control.extend({
	...
	ready: function() {
		...
		var control = this,
			...
			input_default,
			changeAction,
			woovina_range_input_number_timeout;

		// Update the text value
		jQuery('input[type=range]').on('mousedown', function() {

			range 			= jQuery(this);
			range_input 	= range.parent().children('.woovina-range-input');
		...
		function woovina_autocorrect_range_input_number(input_number, timeout) {
			...
			clearTimeout(woovina_range_input_number_timeout);

			woovina_range_input_number_timeout = setTimeout(function() {
		...
		jQuery('input.woovina-range-input').on('change keyup', function() {

			woovina_autocorrect_range_input_number(jQuery(this), 1000);

		}).on('focusout', function() {

			woovina_autocorrect_range_input_number(jQuery(this), 0);

		});

		// Handle the reset button
		jQuery('.woovina-reset-slider').on('click', function() {
