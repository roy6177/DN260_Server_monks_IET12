 ...
 * @return void
 */
function woovina_breadcrumb_trail($args = array()) {
 ...
 * @access public
 */
class WooVina_Breadcrumb_Trail {
	...
	/**
	 * Sets up the breadcrumb trail properties.  Calls the `WooVina_Breadcrumb_Trail::add_items()` method
	 ...
	 *     @type array     $labels         Text labels. @see WooVina_Breadcrumb_Trail::set_labels()
	 *     @type array     $post_taxonomy  Taxonomies to use for post types. @see WooVina_Breadcrumb_Trail::set_post_taxonomy()
	...
	public function get_trail() {
		...
		if(0 < $item_count) {
			...
			$breadcrumb .= '<ul class="trail-items"'. woovina_get_schema_markup('breadcrumb') .'>';
			...
			foreach ($this->items as $item) {
				...
				$item = !empty($matches) ? sprintf('%s<span'. woovina_get_schema_markup('author_name') .'>%s</span>%s', $matches[1], $matches[2], $matches[3]) : sprintf('<span'. woovina_get_schema_markup('author_name') .'>%s</span>', $item);
				...
				$attributes = 'class="' . $item_class . '"'. woovina_get_schema_markup('breadcrumb_list') .'';

				// Build the meta position HTML.
				$meta = sprintf('<meta content="%s"'. woovina_get_schema_markup('position') .' />', absint($item_position));
			...
			// Postion class
			$p_class = '';
			if('' != get_theme_mod('woovina_breadcrumbs_position')) {
				$p_class = ' position-' . get_theme_mod('woovina_breadcrumbs_position');
			}

			// Wrap the breadcrumb trail.
			$breadcrumb = sprintf(
				'<%1$s role="navigation" aria-label="%2$s" class="site-breadcrumbs clr'. $p_class .'"'. woovina_get_schema_markup('breadcrumb_itemprop') .'>%3$s%4$s%5$s</%1$s>',
	...
	protected function set_labels() {
		...
		$defaults = array(
			'aria_label'          => esc_attr_x('Breadcrumbs', 'breadcrumbs aria label', 'woovina'),
			'home'                => esc_html__('Home',                                  'woovina'),
			'error_404'           => esc_html__('404 Not Found',                         'woovina'),
			'archives'            => esc_html__('Archives',                              'woovina'),
			// Translators: %s is the search query. The HTML entities are opening and closing curly quotes.
			'search'              => esc_html__('Search results for &#8220;%s&#8221;',   'woovina'),
			// Translators: %s is the page number.
			'paged'               => esc_html__('Page %s',                               'woovina'),
			// Translators: Minute archive title. %s is the minute time format.
			'archive_minute'      => esc_html__('Minute %s',                             'woovina'),
			// Translators: Weekly archive title. %s is the week date format.
			'archive_week'        => esc_html__('Week %s',                               'woovina'),
	...
	protected function add_minute_hour_archive_items() {
		...
		if(true === $this->args['show_title'])
			$this->items[] = sprintf($this->labels['archive_minute_hour'], get_the_time(esc_html_x('g:i a', 'minute and hour archives time format', 'woovina')));
	}
	...
	protected function add_minute_archive_items() {
		...
		// Add the minute item.
		if(true === $this->args['show_title'])
			$this->items[] = sprintf($this->labels['archive_minute'], get_the_time(esc_html_x('i', 'minute archives time format', 'woovina')));
	}
	...
	protected function add_hour_archive_items() {
		...
		// Add the hour item.
		if(true === $this->args['show_title'])
			$this->items[] = sprintf($this->labels['archive_hour'], get_the_time(esc_html_x('g a', 'hour archives time format', 'woovina')));
	}
	...
	protected function add_day_archive_items() {
		...
		$year  = sprintf($this->labels['archive_year'],  get_the_time(esc_html_x('Y', 'yearly archives date format',  'woovina')));
		$month = sprintf($this->labels['archive_month'], get_the_time(esc_html_x('F', 'monthly archives date format', 'woovina')));
		$day   = sprintf($this->labels['archive_day'],   get_the_time(esc_html_x('j', 'daily archives date format',   'woovina')));
	...
	protected function add_week_archive_items() {
		...
		$year = sprintf($this->labels['archive_year'],  get_the_time(esc_html_x('Y', 'yearly archives date format', 'woovina')));
		$week = sprintf($this->labels['archive_week'],  get_the_time(esc_html_x('W', 'weekly archives date format', 'woovina')));
	...
	protected function map_rewrite_tags($post_id, $path) {
		...
		if(is_array($matches)) {
			...
			foreach ($matches as $match) {
				...
				if('%year%' == $tag)
					$this->items[] = sprintf('<a href="%s">%s</a>', esc_url(get_year_link(get_the_time('Y', $post_id))), sprintf($this->labels['archive_year'], get_the_time(esc_html_x('Y', 'yearly archives date format',  'woovina'))));

				// If using the %monthnum% tag, add a link to the monthly archive.
				elseif('%monthnum%' == $tag)
					$this->items[] = sprintf('<a href="%s">%s</a>', esc_url(get_month_link(get_the_time('Y', $post_id), get_the_time('m', $post_id))), sprintf($this->labels['archive_month'], get_the_time(esc_html_x('F', 'monthly archives date format', 'woovina'))));

				// If using the %day% tag, add a link to the daily archive.
				elseif('%day%' == $tag)
					$this->items[] = sprintf('<a href="%s">%s</a>', esc_url(get_day_link(get_the_time('Y', $post_id), get_the_time('m', $post_id), get_the_time('d', $post_id))), sprintf($this->labels['archive_day'], get_the_time(esc_html_x('j', 'daily archives date format', 'woovina'))));
