...
if ( ! function_exists( 'sinatra_schema_markup' ) ) :
	...
	function sinatra_schema_markup( $location ) {
		echo sinatra_get_schema_markup( $location ); // phpcs:ignore
	}
endif;
...
if ( ! function_exists( 'sinatra_animated_arrow' ) ) :
	...
	function sinatra_animated_arrow( $style = 'right', $type = 'button', $echo = false ) {
		...
		if ( false !== $type ) {
			...
			$button = '
			<button type="' . esc_attr( $type ) . '" class="sinatra-animate-arrow ' . esc_attr( $style ) . '-arrow" aria-hidden="true" role="button">
	...
	function sinatra_excerpt( $length = null, $more = null ) {
		...
		if ( has_excerpt( $post->ID ) ) {
			...
			if ( strpos( $post->post_content, '<!--more-->' ) ) {
				$output = apply_filters( 'the_content', get_the_content() ); // phpcs:ignore
			} else {

				if ( null === $length ) {
					$length = apply_filters( 'excerpt_length', intval( sinatra_option( 'excerpt_length' ) ) ); // phpcs:ignore
				}

				if ( null === $more ) {
					$more = apply_filters( 'excerpt_more', sinatra_option( 'excerpt_more' ) ); // phpcs:ignore
				}
...
if ( ! function_exists( 'sinatra_entry_meta_date' ) ) :
	...
	function sinatra_entry_meta_date( $args = array() ) {
		...
		// Icon.
		$icon = sinatra_get_meta_icon( 'date', '<i class="si-icon si-clock" aria-hidden="true"></i>' );
		...
		$time_string = sprintf(
			...
			wp_kses_post( $icon ) . esc_html( get_the_date( $args['date_format'] ) ),
...
if ( ! function_exists( 'sinatra_entry_meta_comments' ) ) :
	...
	function sinatra_entry_meta_comments() {
		...
		$icon = sinatra_get_meta_icon( 'comments', '<i class="si-icon si-message-square" aria-hidden="true"></i>' );

		if ( ! post_password_required() && ( comments_open() || get_comments_number() ) ) {
			echo '<span class="comments-link">';

			comments_popup_link( wp_kses_post( $icon ) . esc_html__( 'No Comments', 'sinatra' ), wp_kses_post( $icon ) . esc_html__( '1 Comment', 'sinatra' ), wp_kses_post( $icon ) . esc_html__( '% Comments', 'sinatra' ), 'comments-link' );
...
if ( ! function_exists( 'sinatra_entry_meta_category' ) ) :
	...
	function sinatra_entry_meta_category( $sep = ', ', $show_icon = true, $return = false ) {
		...
		// Icon.
		$icon = $show_icon ? sinatra_get_meta_icon( 'category', '<i class="si-icon si-bookmark" aria-hidden="true"></i>' ) : '';
...
if ( ! function_exists( 'sinatra_entry_meta_tag' ) ) :
	...
	function sinatra_entry_meta_tag( $before = '<span class="cat-links"><span>', $sep = ', ', $after = '</span></span>', $id = 0, $show_icon = true ) {
		...
		$icon = $show_icon ? sinatra_get_meta_icon( 'tags', '<i class="si-icon si-tag" aria-hidden="true"></i>' ) : '';

		// Add icon.
		$before = $before . wp_kses_post( $icon );
...
if ( ! function_exists( 'sinatra_post_media' ) ) :
	...
	function sinatra_post_media( $post_format = false ) {
		echo sinatra_get_post_media( $post_format ); // phpcs:ignore
	}
endif;
				...
				>
					...
					<header class="comment-header">
						...
						<div class="comment-actions">
							...
							<div class="reply">
								<?php echo $sinatra_comment_reply_link; // phpcs:ignore ?>
							</div>
						</div>
