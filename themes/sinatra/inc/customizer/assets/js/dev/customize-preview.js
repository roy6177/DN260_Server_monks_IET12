...
;( function( $ ) {
	...
	function sinatra_typography_field_css( selector, setting ) {
		...
		if ( 'font-size-tablet' in setting && setting['font-size-tablet'] ) {
			css += '@media only screen and (max-width: 768px) {' + 
						selector + '{' +
							'font-size: ' + setting['font-size-tablet'] + setting['font-size-unit'] + ';' +
						'}' +
					'}';
		}
		...
		if ( 'line-height-tablet' in setting && setting['line-height-tablet'] ) {
			...
			css += '@media only screen and (max-width: 768px) {' + 
						selector + '{' +
							'line-height:' + setting['line-height-tablet'] + ';' +
						'}' +
					'}';
		}
		...
		if ( 'font-size-mobile' in setting && setting['font-size-mobile'] ) {
			css += '@media only screen and (max-width: 480px) {' +
						selector + '{' +
							'font-size: ' + setting['font-size-mobile'] + setting['font-size-unit'] + ';' +
						'}' +
					'}';
		}
		...
		if ( 'line-height-mobile' in setting && setting['line-height-mobile'] ) {
			css += '@media only screen and (max-width: 480px) {' +
						selector + '{' +
							'line-height:' + setting['line-height-mobile'] + ';' +
						'}' +
					'}';
		}
		...
		if ( 'background' === type ) {
			...
			if ( 'color' === bg_type ) {
				...
				if ( 'linear' === setting['gradient-type'] ) {
					css += 'background: -webkit-linear-gradient(' + setting['gradient-linear-angle'] + 'deg, ' + setting['gradient-color-1'] + ' ' + setting['gradient-color-1-location'] + '%, ' + setting['gradient-color-2'] + ' ' + setting['gradient-color-2-location'] + '%);' +
					...
					css += 'background: -webkit-radial-gradient(' + setting['gradient-position'] + ', circle, ' + setting['gradient-color-1'] + ' ' + setting['gradient-color-1-location'] + '%, ' + setting['gradient-color-2'] + ' ' + setting['gradient-color-2-location'] + '%);' +
	...
	api( 'sinatra_sidebar_width', function( value ) {
		value.bind( function( newval ) {
			...
			var style_css  = '#secondary { width: ' + newval.value + '%; }';
				style_css += 'body:not(.sinatra-no-sidebar) #primary { ' +
					'max-width: ' + ( 100 - parseInt( newval.value ) ) + '%;' +
				'};';
	...
	api( 'sinatra_single_narrow_container_width', function( value ) {
		value.bind( function( newval ) {
			...
			style_css += '.single-post.narrow-content .entry-content > :not([class*="align"]):not([class*="gallery"]):not(.wp-block-image):not(.quote-inner):not(.quote-post-bg), ' +
					...
					'.single-post.narrow-content.sinatra-no-sidebar .si-breadcrumbs > .si-container > nav {' +
						'max-width: ' + parseInt( newval.value ) + 'px; margin-left: auto; margin-right: auto; ' +
					'}';

			style_css += '.single-post.narrow-content .author-box, ' +
						 '.single-post.narrow-content .entry-content > .alignwide { ' +
						 	'max-width: ' + ( parseInt( newval.value ) + 70 ) + 'px;' +
						 '}';
	...
	api( 'sinatra_main_nav_font_color', function( value ) {
		value.bind( function( newval ) {
			...
			style_css += '#sinatra-header-inner .sinatra-nav > ul > li > a:hover, ' +
	...
	api( 'sinatra_top_bar_text_color', function( value ) {
		value.bind( function( newval ) {
			...
			// Link color.
			style_css += '.si-topbar-widget__text a, ' +
			             '.si-topbar-widget .sinatra-nav > ul > li > a, ' +
			             '.si-topbar-widget__socials .sinatra-social-nav > ul > li > a, ' +
			             '#sinatra-topbar .si-topbar-widget__text .si-icon { color: ' + newval['link-color'] + '; }';	
			
			// Link hover color.
			style_css += '#sinatra-topbar .sinatra-nav > ul > li > a:hover, ' +
						 ...
						 '#sinatra-topbar .sinatra-social-nav > ul > li > a .si-icon.bottom-icon { color: ' + newval['link-hover-color'] + '; }';	
	...
	api( 'sinatra_header_text_color', function( value ) {
		value.bind( function( newval ) {
			...
			if ( newval['link-color'] ) {
				style_css += '#sinatra-header, ' +
							 '.si-header-widgets a:not(.si-btn), ' +
							 '.sinatra-logo a,' +
							 '.si-hamburger { color: ' + newval['link-color'] + '; }';
				style_css += '.hamburger-inner,' +
							 '.hamburger-inner::before,' +
							 '.hamburger-inner::after { background-color: ' + newval['link-color'] + '; }';
			}
			
			// Link hover color.
			if ( newval['link-hover-color'] ) {
				style_css += '.si-header-widgets a:not(.si-btn):hover, ' +
							 ...
							 '#sinatra-header-inner .sinatra-nav > ul > li.page_item_has_children:hover > a,' +
							 '#sinatra-header-inner .sinatra-nav > ul > li.current_page_item > a,' +
							 '#sinatra-header-inner .sinatra-nav > ul > li.current_page_ancestor > a { color: ' + newval['link-hover-color'] + '; }';
				
				style_css += '.si-hamburger:hover .hamburger-inner,' +
							 ...
							 '.is-mobile-menu-active .si-hamburger .hamburger-inner::after { background-color: ' + newval['link-hover-color'] + '; }';
			}
	...
	api( 'sinatra_hero_hover_slider_overlay', function( value ) {
		value.bind( function( newval ) {
			...
			$hero.removeClass( function (index, className) {
	 ...
	 * Dropdown indicators.
	 */
	api( 'sinatra_main_nav_sub_indicators', function( value ) {
		value.bind( function( newval ) {
			if ( newval ) {
				$body.addClass( 'sinatra-with-dropdown-indicators' );
			} else {
				$body.removeClass( 'sinatra-with-dropdown-indicators' );
			}
		});
	});
	...
	api( 'sinatra_pre_footer_cta_background', function( value ) {
		value.bind( function( newval ) {
			...
			if ( 'color' === newval['background-type'] ) {
				...
				style_css += '.si-pre-footer-cta-style-1 #si-pre-footer .si-flex-row::after,' +
			 				 '.si-pre-footer-cta-style-2 #si-pre-footer::after' +
			 				 '{ background-image: none; }';
			} else {
				style_css += sinatra_design_options_css( '.si-pre-footer-cta-style-1 #si-pre-footer .si-flex-row::after, .si-pre-footer-cta-style-2 #si-pre-footer::after', newval, 'background' );
			}

			if ( 'image' === newval['background-type'] && newval['background-color-overlay'] && newval['background-image'] ) {
				style_css += '.si-pre-footer-cta-style-1 #si-pre-footer .si-flex-row::before,' +
			 				 '.si-pre-footer-cta-style-2 #si-pre-footer::before' +
			 				 '{ background-color: ' + newval['background-color-overlay'] + '; }';
			}
	...
	api( 'sinatra_accent_color', function( value ) {
	 	value.bind( function( newval ) {
	 		...
	 		// Background colors.
	 		style_css = '.si-header-widgets .si-cart .si-cart-count,' +
			...
			style_css += '.si-btn:hover, ' +
						 ...
						 '.woocommerce #review_form #respond .form-submit input:hover { ' +
						 	'background-color: ' + sinatra_luminance( newval, 0.15 ) + ';' +
						 '}';

			// Hover accent color.
			style_css += '.wc-layered-nav-rating a:hover .star-rating span:before { ' +
						 	'color: ' + sinatra_luminance( newval, 0.15 ) + ';' +
						 '}';
			...
			style_css += 'code, ' +
						 ...
						 'span.highlight, ' +
						 'tt { ' +
							'background-color: ' + sinatra_hex2rgba( newval, 0.12 ) + ';' +
						 '}';

			style_css += 'code.block { ' +
							'background-color: ' + sinatra_hex2rgba( newval, 0.075 ) + ';' +
						 '}';

			// Colors.
			style_css += '.content-area a:not(.si-btn):not(.wp-block-button__link),' +
			...
			// Border color.
			style_css += '#comments .comment-actions .reply a:hover, ' +
			...
			// Border bottom color.
			style_css += '#masthead .si-header-widgets .dropdown-item::after, ' +
			...
			// Border top color.
			style_css += '.si-header-widgets .dropdown-item, ' +
						 '.site .woocommerce-info, ' +
						 '.preloader-1 > div, ' +
						 '.si-header-element.sinatra-nav .sub-menu {' +
						 	'border-top-color: ' + newval + ';' +
						 '}';

			// Fill color.
			style_css += '.sinatra-animate-arrow:hover .arrow-handle, ' +
			...
			// Box shadow.
			style_css += '.si-input-supported input[type=checkbox]:focus:hover { ' +
						  	'box-shadow: inset 0 0 0 2px ' + newval + '; ' +
						  '}';

			// Gradient.
			style_css += '.si-pre-footer-cta-style-1 #si-pre-footer .si-flex-row::before,' +
						 '.si-pre-footer-cta-style-2 #si-pre-footer::before { ' +
						 'background: linear-gradient(to right, ' + sinatra_hex2rgba( newval, 0.9 ) + ' 0%, ' + sinatra_hex2rgba( newval, 0.82 ) + ' 35%, ' + sinatra_hex2rgba( newval, 0.4 ) + ' 100% );' +
						 '-webkit-gradient(linear, left top, right top, from(' + sinatra_hex2rgba( newval, 0.9 ) + '), color-stop(35%, ' + sinatra_hex2rgba( newval, 0.82 ) + '), to(' + sinatra_hex2rgba( newval, 0.4 ) + ')); }';
	...
	api( 'sinatra_boxed_content_background_color', function( value ) {
	 	value.bind( function( newval ) {
			...
			if ( newval ) {
		 		style_css =  '.sinatra-layout__boxed #page, ' +
							 ...
							 '.sinatra-layout__boxed-separated.category .sinatra-article { background-color: ' + newval + '; }';

				style_css += '@media screen and (max-width: 960px) { ' +
								'.sinatra-layout__boxed-separated #page { background-color: ' + newval + '; }' +
							 '}';
		 	}
	...
	api( 'sinatra_content_text_color', function( value ) {
	 	value.bind( function( newval ) {
			...
			if ( newval ) {
				...
				'.woocommerce div.product span.price del { color: ' + sinatra_hex2rgba( newval, 0.75 ) + '; }';
		 	}
	...
	api( 'sinatra_content_link_hover_color', function( value ) {
	 	value.bind( function( newval ) {
			...
			if ( newval ) {
		 		// Content link hover.
		 		style_css += '.content-area a:not(.si-btn):not(.wp-block-button__link):hover, ' +
	...
	api( 'sinatra_headings_color', function( value ) {
	 	value.bind( function( newval ) {
			...
			if ( newval ) {
		 		style_css = 
		 		'h1, h2, h3, h4, h5, h6, .h1, .h2, .h3, .sinatra-logo .site-title, .error-404 .page-header h1 { ' +
					'color: ' + newval + ';' +
				'}';
		 	}
	...
	api( 'sinatra_footer_text_color', function( value ) {
		...
		var $footer = $( '#sinatra-footer' ),
	 	...
	 	value.bind( function( newval ) {
			...
			// Link hover color.
			style_css += '#colophon a:hover, #colophon li.current_page_item > a, #colophon .sinatra-social-nav > ul > li > a .si-icon.bottom-icon ' +
						 '{ color: ' + newval['link-hover-color'] + '; }';
	...
	api( 'sinatra_footer_border', function( value ) {
		value.bind( function( newval ) {
			...
			if ( newval['border-top-width'] ) {
				style_css += '#colophon { ' +
								'border-top-width: ' + newval['border-top-width'] + 'px;' +
								'border-top-style: ' + newval['border-style'] + ';' +
								'border-top-color: ' + newval['border-color'] + ';' +
							'}';
			}

			if ( newval['border-bottom-width'] ) {
				style_css += '#colophon { ' +
								'border-bottom-width: ' + newval['border-bottom-width'] + 'px;' +
								'border-bottom-style: ' + newval['border-style'] + ';' +
								'border-bottom-color: ' + newval['border-color'] + ';' +
							'}';
			}
	...
	api( 'sinatra_container_width', function( value ) {
	 	value.bind( function( newval ) {
	 		...
	 		style_css =  '.si-container,' +
						 '.alignfull > div { ' +
						 	'max-width: ' + newval.value + 'px;' +
						 '}';

			style_css += '.sinatra-layout__boxed #page { max-width: ' + ( parseInt( newval.value) + 100 ) + 'px; }';
	...
	api( 'sinatra_tsp_header_font_color', function( value ) {
		value.bind( function( newval ) {
			...
			// Link color.
			if ( newval['link-color'] ) {
				style_css += '.si-tsp-header #sinatra-header, ' +
							 '.si-tsp-header .si-header-widgets a:not(.si-btn), ' +
							 '.si-tsp-header .sinatra-logo a,' +
							 '.si-tsp-header .si-hamburger, ' +
							 '.si-tsp-header #sinatra-header-inner .sinatra-nav > ul > li > a { color: ' + newval['link-color'] + '; }';
				style_css += '.si-tsp-header .hamburger-inner,' +
							 '.si-tsp-header .hamburger-inner::before,' +
							 '.si-tsp-header .hamburger-inner::after { background-color: ' + newval['link-color'] + '; }';
			}
			
			// Link hover color.
			if ( newval['link-hover-color'] ) {
				style_css += '.si-tsp-header .si-header-widgets a:not(.si-btn):hover, ' +
							 ...
							 '.si-tsp-header #sinatra-header-inner .sinatra-nav > ul > li.current_page_ancestor > a { color: ' + newval['link-hover-color'] + '; }';
				
				style_css += '.si-tsp-header .si-hamburger:hover .hamburger-inner,' +
							 ...
							 '.is-mobile-menu-active .si-tsp-header .si-hamburger .hamburger-inner::after { background-color: ' + newval['link-hover-color'] + '; }';
			}
	...
	api( 'sinatra_page_header_background', function( value ) {
		value.bind( function( newval ) {
			...
			var style_css = sinatra_design_options_css( '.page-header', newval, 'background' );
	...
	api( 'sinatra_page_header_text_color', function( value ) {
		value.bind( function( newval ) {
			...
			style_css += '.page-header .si-page-header-description' +
						 '{ color: ' + sinatra_hex2rgba( newval['text-color'], 0.75 ) + '}';

			// Link color.
			style_css += '.page-header .si-breadcrumbs a' +
						 '{ color: ' + newval['link-color'] + '; }';

			style_css += '.page-header .si-breadcrumbs span,' +
						 '.page-header .breadcrumb-trail .trail-items li::after, .page-header .si-breadcrumbs .separator' +
						 '{ color: ' + sinatra_hex2rgba( newval['link-color'], 0.75 ) + '}';
	...
	api( 'sinatra_headings_font', function( value ) {
		...
		value.bind( function( newval ) {
			...
			selector += ', h4, .woocommerce .cart_totals h2, .woocommerce .cross-sells > h4, .woocommerce #reviews #respond .comment-reply-title';
			selector += ', h5, h6';
	...
	api( 'sinatra_h4_font', function( value ) {
		value.bind( function( newval ) {
			...
			var style_css = sinatra_typography_field_css( 'h4, .woocommerce .cart_totals h2, .woocommerce .cross-sells > h4, .woocommerce #reviews #respond .comment-reply-title', newval );
	...
	api( 'sinatra_heading_em_font', function( value ) {
		value.bind( function( newval ) {
			...
			var style_css = sinatra_typography_field_css( 'h1 em, h2 em, h3 em, h4 em, h5 em, h6 em, .h1 em, .h2 em, .h3 em, .sinatra-logo .site-title em, .error-404 .page-header h1 em', newval );
	...
	var $btn_hover_selectors = '.si-btn:hover, ' +
							   '.si-btn:focus, ' +
	...
	api( 'sinatra_primary_button_border_radius', function( value ) {
	 	value.bind( function( newval ) {
	 		...
	 		if ( newval ) {
	 			style_css = $btn_selectors + ' { ' +
	 				'border-top-left-radius: ' + newval['top-left'] + 'px;' +
	 				'border-top-right-radius: ' + newval['top-right'] + 'px;' +
	 				'border-bottom-left-radius: ' + newval['bottom-left'] + 'px;' +
	 				'border-bottom-right-radius: ' + newval['bottom-right'] + 'px; }';
	...
	var $btn_sec_hover_selectors = '.btn-secondary:hover, ' +
								   '.btn-secondary:focus, ' +
	...
	if ( api.selectiveRefresh ) {
		...
		api.selectiveRefresh.bind( 'partial-content-rendered', function( placement ){
			...
			// Hero Hover Slider.
			if ( 'sinatra_hero_hover_slider_post_number' === placement.partial.id || 
				 'sinatra_hero_hover_slider_elements' === placement.partial.id ) {
