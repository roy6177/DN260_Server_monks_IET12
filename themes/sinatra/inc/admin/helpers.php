...
function sinatra_print_notice( $args ) {
	...
	<div id="<?php echo esc_attr( $args['message_id'] ); ?>" class="notice sinatra-notice notice-<?php echo esc_attr( $args['type'] ); echo esc_attr( $sinatra_is_dismissible ); ?>">
...
function sinatra_print_admin_icon( $icon = 'info', $tooltip = '', $tooltip_position = 'right-tooltip' ) {
	...
	if ( 'warning' === $icon ) {
		echo '<i class="sinatra-warning-icon">' . $svg_icon . $tooltip . '</i>'; // phpcs:ignore
	} elseif ( 'info' === $icon ) {
		echo '<i class="sinatra-info-icon">' . $svg_icon . $tooltip . '</i>'; // phpcs:ignore
	}
}
