...
class ThemeUpdateChecker {
	...
	public function requestUpdate($queryArgs = array()){
		$key				= 'woovina_' . $this->theme;
		$license 			= get_option('edd_license_details');
		$license_details 	= (isset($license) && isset($license[$key])) ? $license[$key] : false;
		$license_key		= (isset($license_details) && isset($license_details->checksum)) ? $license_details->checksum : '';
		
		//Query args to append to the URL. Themes can add their own by using a filter callback (see addQueryArgFilter()).
		$queryArgs['installed_version'] = $this->getInstalledVersion();
		$queryArgs['edd_action'] 		= 'request_update';
		$queryArgs['item_type']  		= 'woovina_theme';
		$queryArgs['item_name']  		= $this->theme;
		$queryArgs['license'] 			= $license_key;
		$queryArgs['url'] 				= home_url();
